...
def create_korean_homework_agent() -> StateGraph:
    ...
    async def generate_detailed_homework_node(state: AgentState):
        ...
        prompt = ChatPromptTemplate.from_messages([
            ...
            "숙제 안내문은 다음 HTML 태그만 사용하여 작성해야 합니다: <p>, <a>, <b>, <h1>, <h2>, <h3>, <ul>, <li>, <br>\n"
            "<a> 태그의 href 속성만 유지되며, 다른 모든 HTML 속성은 제거됩니다.\n\n"
            "숙제 안내문에는 다음 내용이 포함되어야 합니다:\n"
            "1. 숙제 제목 (흥미를 유발하는 창의적인 제목) - <h1> 또는 <h2> 태그 사용\n"
            "2. 명확한 글쓰기 지시사항 (무엇을, 어떻게 써야 하는지) - <p> 태그 사용\n"
            "3. 포함되어야 할 내용 또는 고려사항 (필요시 키워드 활용 안내 포함) - <ul>와 <li> 태그 사용\n"
            "4. 예상 분량 (5줄 이내) - <p> 태그 사용\n\n"
            "모든 내용은 한국어로 자연스럽게 작성해주세요.\n"
            "예시 숙제 안내문 (학생에게 제시될 글쓰기 과제 부분):\n"
            "--------------------------------------------------\n"
            "<h1>✨ 나의 특별한 생일 이야기 ✨</h1>\n\n"
            "<p>친구들, 안녕하세요! 오늘은 여러분의 생일에 대한 이야기를 나눠볼 거예요.</p>\n\n"
            "<ul>\n"
            "<li>🎂 생일이 언제예요?</li>\n"
            "<li>🎁 그리고 생일에 받고 싶은 선물이나 하고 싶은 것을 얘기해볼까요?</li>\n"
            "<li>🎉 이미 생일이 지났다면 생일에 했던 일, 받았던 선물을 얘기해볼까요?</li>\n"
            "</ul>\n\n"
            "<p>자유롭게 여러분의 생각이나 경험을 표현해주세요.</p>\n"
            "<p>✏️ 5줄 이상으로 작성해봐요 😄</p>\n"
            "--------------------------------------------------"
            ),
